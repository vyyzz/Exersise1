...
TEST_CASE("linked_list testing") {
    ...
    SUBCASE("delete_linked_list") {
        // Check if deleting a non-empty linked list results in null
        delete_linked_list(linked_list_one_elt);
        CHECK_EQ(linked_list_one_elt, nullptr);

        // Check if deleting an empty linked list results in null
        delete_linked_list(zero_linked_list_elts_null_node);
        CHECK_EQ(zero_linked_list_elts_null_node, nullptr);

        // Check if deleting a linked list with multiple elements results in null
        delete_linked_list(linked_list_two_elts);
        CHECK_EQ(linked_list_two_elts, nullptr);
    }
}
